...
suite('TestService Test Suite', () => {
	...
	test('runTest should create a terminal with the correct parameters', () => {
		...
		assert.strictEqual(sendTextStub.firstCall.args[0], 'go test -v . -run "/Test_Feature/Test_Scenario$"');
	});
	...
	test('runTest should reuse existing terminal if available', () => {
		...
		assert.strictEqual(existingSendTextStub.firstCall.args[0], 'go test -v . -run "/Test_Feature/"');
	});
	...
	test('runTest should handle feature name only', () => {
		...
		// Verify command was sent to terminal
		assert.strictEqual(sendTextStub.calledOnce, true);
		assert.strictEqual(sendTextStub.firstCall.args[0], 'go test -v . -run "/Test_Feature/"');
	});
	...
	test('runTest should handle scenario name only', () => {
		...
		// Verify command was sent to terminal
		assert.strictEqual(sendTextStub.calledOnce, true);
		assert.strictEqual(sendTextStub.firstCall.args[0], 'go test -v . -run "//Test_Scenario$"');
	});
});