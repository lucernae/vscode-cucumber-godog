...
function runTest(filePath: string, featureName?: string, scenarioName?: string) {
	...
	// check if programWorkingDirectory is relative to the feature file
	...
	if (programWorkingDirectory.startsWith('.')) {
		...
		workingDir = path.resolve(relativePath);
	} else {
		// if not, use the programWorkingDirectory as is
		workingDir = path.resolve(dirPath, programWorkingDirectory);
	}
